...
const HOUSE_NOTICES: { [key: string]: { title: string, content: string } } = {
  "래번클로": {
    title: `2위의 아쉬움을 넘어서, 지적인 몰입!`,
    content: `래번클로 학우 여러분, 지난주 우리는 근소한 차이로 2위를 기록했습니다. 1위를 차지한 후플푸프의 성실함은 본받을 만하나, 우리 래번클로의 무기는 깊이 있는 통찰과 집중력입니다! 
플리트윅 교수님께서는 이번 설 연휴 3일의 자율학습이 우리가 다시 1위로 올라설 논리적인 지름길이라고 말씀하셨습니다.

자율학습 9시간 달성은 규정상의 상점을 획득할 수 있는 확실한 방법입니다. 연휴라고 해서 사고의 끈을 놓지 마세요! 
도서관의 정적 속에서 9시간 동안 학문의 바다에 빠져드는 경험은 그 자체로 상점 이상의 가치가 있을테니까요.

독수리 문고리가 '휴식과 나태의 차이는 무엇인가?'라는 질문을 던지기 시작했습니다. 철학적인 답변과 함께 기숙사에 당당히 입장하여, 다음주에는 반드시 푸른 깃발을 연회장 중앙에 세워봅시다!

`
  },

  "그리핀도르": {
    title: `사자의 기개를 증명할 진정한 용기`,
    content: `그리핀도르 학우 여러분! 3위라는 순위는 우리 사자들에게 어울리지 않는 성적입니다. 하지만 3위도 나쁘진 않죠. 다시 1위를 할 수 있는 기회가 생겼으니까요!

진정한 용기는 화려한 결투에서만 나오는 것이 아니라, 책상 앞에 앉아 엉덩이를 붙이고 버티는 인내심에서도 나온다는 것을 명심하세요!
9시간을 견뎌낸 학생들에게 부여되는 상점은 우리를 다시 1위로 만들어 줄 수 있을겁니다. 
네빌 롱보텀 교수님께서도 여러분을 응원하며 휴게실 벽난로를 평소보다 더 뜨겁게 지펴두셨습니다. 떡국 한 그릇 든든하게 먹고 지팡이 대신 깃펜을 드십시오! 
이번 주말, 순위판이 뒤집히는 짜릿한 역전극의 주인공이 바로 우리가 될 것입니다. 그리핀도르, 돌격합시다!

`
  },

  "슬리데린": {
    title: `은밀하고 완벽한 반격을 설계하십시오!`,
    content: `슬리데린 학우 여러분, 현재 4위라는 순위는 우리 기숙사의 자존심에 큰 상처입니다. 하지만 슬리데린은 패배에 좌절하지 않고, 패배를 분석하여 승리의 발판으로 삼는 곳입니다. 
이번 설 연휴 3일간의 자율학습은 슬리데린이 다시 권력을 잡기 위한 완벽한 기회입니다.

벌점이 없다고 방종에 빠진 타 기숙사 학생들이 연휴의 나태함에 취해 있을 때, 우리는 철저하게 하루 9시간을 채워 정식 상점을 독식할 것입니다. 
우리의 집요한 열망을 책 위로 쏟으십시오.  9시간 동안의 침묵과 몰입은 우리를 다시 위대하게 만들 것입니다.
슬러그혼 교수님께서도 명절 기간 9시간 학습을 달성한 학생들을 유심히 지켜보시겠다고 암시하셨습니다. 

이번 주 새로운 암호는 '역전의 야망'입니다. 
이 암호를 속삭이며 입구에 들어설 때마다, 4위에서 1위로 올라갈 우리의 차가운 계획을 되새기십시오!
...
`
  ...
  "후플푸프": {
    title: `노란 깃발의 영광! 
    명절에도 식지 않는 우리의 성실 미학`,
    content: `자랑스러운 후플푸프 학우 여러분! 우리가 1위의 자리에 올랐습니다. 
이번 승리는 단순히 운이 아니라, 신입생들을 따뜻하게 챙기고 온실 일을 자기 일처럼 도왔던 우리 기숙사만의 다정함이 만든 결과입니다. 스프라우트 교수님께서도 여러분이 정말 대견하다며 휴게실에 특별히 따뜻한 생강차를 준비해 주셨습니다.

이번 설 연휴 3일간은 자율학습 기간입니다. 비록 벌점은 없지만, 우리 후플푸프의 강점은 누가 보지 않아도 자기 자리를 지키는 것 아니겠어요? 
하루 9시간 자율학습이라는 목표에 도전해 봅시다. 9시간이 길게 느껴진다면 학우들과 함께 모여 공부하세요. 상점을 차곡차곡 쌓아 다음 주에도 이 우승 깃발을 지켜냅시다! 
연휴의 끝에는 주방 요정들이 준비한 특별한 명절 음식이 기다리고 있을 테니, 조금만 더 힘을 냅시다!

...
export default function PlannerPage() {
  ...
  const getMonday = (offsetDays = 0) => {
    ...
    const monday = new Date(now.setDate(diff + offsetDays));
  ...
  const addTodo = (day: string) => {
    ...
    newData[day] = [...newData[day], { id: Date.now().toString(), subject: subjects.find(s => s !== "") || "과목없음", content: "", completed: false }];
    saveData(newData);
  };
  ...
  const theme = {
    ...
    input: isDarkMode ? 'bg-white/5 border-white/10 text-white focus:bg-white/10' : 'bg-slate-50 border-slate-200 text-slate-800 focus:bg-white',
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8">
          <div className="w-full md:w-auto">
            ...
            <p className={`text-[11px] font-black uppercase tracking-[0.3em] ${theme.accent}`}>Goal: {examDate || "결전의 시험 날짜를 설정하세요"}</p>
          </div>
          ...
          <div className={`p-6 rounded-[2rem] border w-full md:w-[400px] ${theme.card}`}>
            <div className="flex justify-between items-center mb-4">
              ...
              <input 
                ...
                className={`text-[10px] font-bold p-1.5 rounded-lg outline-none border ${theme.input}`}
              />
            </div>
              ...
              <div key={day} className={`rounded-[2.5rem] border transition-all duration-500 overflow-hidden ${theme.card} ${isOpen ? 'ring-2 ring-blue-500/20' : ''}`}>
                ...
                >
                  ...
                  {!isPastWeek && (
                    <button 
                      ...
                      className="opacity-0 group-hover:opacity-100 bg-blue-600 text-white text-[10px] font-black px-4 py-2 rounded-full shadow-lg transition-all active:scale-95"
                    >
                      + ADD TASK
                    </button>
                  )}
                ...
                {isOpen && (
                  <div className="p-6 pt-2 space-y-3 border-t border-white/5 animate-in fade-in duration-500">
                    {dayTodos.length === 0 && (
                      <div className="text-center py-10 opacity-20 text-xs font-bold tracking-widest">NO PLANS RECORDED</div>
                    )}
                    {dayTodos.map((todo) => (
                      <div key={todo.id} className={`flex items-center gap-3 p-2 rounded-2xl transition-all ${todo.completed ? 'opacity-40' : ''}`}>
                        <input 
                          type="checkbox" 
                          checked={todo.completed}
                          onChange={(e) => updateTodo(day, todo.id, 'completed', e.target.checked)}
                          disabled={isPastWeek}
                          className="w-5 h-5 rounded-lg border-2 border-slate-500 appearance-none checked:bg-blue-500 checked:border-blue-500 cursor-pointer transition-all"
                        />
                        ...
                        <select
                          ...
                          disabled={isPastWeek}
                          className={`text-[11px] font-black p-2 rounded-xl border outline-none transition-all ${theme.input}`}
                        >
                          {subjects.filter(s => s !== "").map((s, i) => <option key={i} value={s}>{s}</option>)}
                          {subjects.every(s => s === "") && <option>과목 설정</option>}
                        </select>
                        ...
                        <input
                          ...
                          onBlur={(e) => updateTodo(day, todo.id, 'content', e.target.value)}

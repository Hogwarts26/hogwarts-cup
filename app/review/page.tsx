...
export default function ReviewPage() {
  ...
  const calcGroupStats = () => {
    ...
    const totalSafePace = Math.ceil(totalRemaining / workDays);

    return {
      dday, workDays, totalRemaining, totalSafePace,
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <Link href="/" style={{
            ...
            border: '1.5px solid #e0dcd6', borderRadius: 999, padding: '6px 12px'
          }}>
            ← 돌아가기
          </Link>
          <span style={{ fontSize: '1.5rem' }}>📚</span>
          <h1 style={{ fontSize: '1.3rem', fontWeight: 900, letterSpacing: -0.5, color: '#1a1a2e' }}>
            기출회독 시뮬레이터
          </h1>
        </div>
        <div style={{
          fontFamily: "'JetBrains Mono', monospace", fontSize: '0.82rem',
          color: '#6b7280', background: '#fff', padding: '6px 12px',
          borderRadius: 999, border: '1px solid #e0dcd6'
        }}>
          {clockStr}
        </div>
      </div>
      ...
      <div style={{
        ...
        {subjects.length < 8 && (
          <button onClick={() => setShowModal(true)} style={{
            ...
            border: '1.5px dashed #e0dcd6', background: 'transparent',
      ...
      {!subj ? (
        ...
        <>
          {/* ── 통합 계산 모드 선택 (과목 2개 이상일 때만) ── */}
          {subjects.length > 1 && (
            <div style={{
              background: '#fff', borderRadius: 16, padding: '14px 20px',
              marginBottom: 14, border: '1px solid #e0dcd6'
            }}>
              <div style={{
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {[
                  ...
                  <button key={key} onClick={() => setGroupMode(key as any)} style={{
                    ...
                    border: '1.5px solid',
                    background: groupMode === key ? '#1a1a2e' : '#f0ede8',
                    color: groupMode === key ? '#fff' : '#6b7280',
                    borderColor: groupMode === key ? '#1a1a2e' : '#e0dcd6',
                    transition: 'all 0.15s'
                  }}>
                    {label}
                  </button>
                ...
                {groupStats.perSubject.map((ps: any) => (
                  <div key={ps.name} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{
                        width: 8, height: 8, borderRadius: '50%', background: ps.color, flexShrink: 0
                      }} />
                    ...
                    <span style={{ fontSize: '0.95rem', fontWeight: 900, color: ps.color }}>
                      {ps.safePace}p/일
                    </span>
                  </div>
                ))}
                ...
                <div key={field}>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      background: '#f0ede8', outline: 'none', boxSizing: 'border-box'
                    }}
                  />
                </div>
              ))}
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1a1a2e', marginBottom: 4 }}>
            ...
            <div style={{ fontSize: '0.82rem', color: '#1a1a2e', marginBottom: 3 }}>
              • D-Day: {s.dday <= 0 ? '마감 초과' : s.dday}
            </div>
            <div style={{ fontSize: '0.82rem', color: '#1a1a2e', marginBottom: 16 }}>
              • 오늘 입력: {s.todayDone}p
            </div>
            ...
            <div style={{
              ...
              {[
                ...
                <div key={label} style={{
                  background: hi ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.7)',
                  borderRadius: 10, padding: '10px 8px', textAlign: 'center',
                  border: `1.5px solid ${hi ? 'rgba(0,0,0,0.15)' : 'rgba(0,0,0,0.06)'}`,
                  boxShadow: hi ? '0 2px 8px rgba(0,0,0,0.08)' : 'none'
                }}>
                  <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#6b7280', marginBottom: 4 }}>
                    {label}
                  </div>
                  <div style={{ fontSize: '1.05rem', fontWeight: 900, color: hi ? sc.text : '#1a1a2e' }}>
                    {value}
                  </div>
                </div>
              ))}
            ...
            <div style={{
              display: 'flex', justifyContent: 'space-between',
              fontSize: '0.78rem', fontWeight: 700, color: '#6b7280', marginBottom: 6
            }}>
              <span>전체 완료율</span>
              <span>{s.pct}%</span>
            </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.82rem', color: '#6b7280', marginTop: 10 }}>
              누적: <strong style={{ color: '#1a1a2e' }}>{s.totalDone}p</strong>
              {' '}/ 남은: <strong style={{ color: '#1a1a2e' }}>{s.remaining}p</strong>
              {' '}({s.pct}%) &nbsp;|&nbsp; 오늘 입력:{' '}
              <strong style={{ color: '#1a1a2e' }}>{s.todayDone}p</strong>
            </div>
          </div>
            ...
            {(subj.logs || []).length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {[...(subj.logs || [])].reverse().map((log: any, i: number) => (
                  <div key={i} style={{
                    display: 'flex', justifyContent: 'space-between',
                    alignItems: 'center', background: '#f0ede8',
                    ...
                    <span style={{ fontWeight: 700, color: subj.color }}>
                      +{log.pages}p
                    </span>
                    <span
                      onClick={() => deleteLog((subj.logs || []).length - 1 - i)}
                      style={{ cursor: 'pointer', color: '#ccc', fontSize: '1rem', padding: '0 4px' }}
                    >
                      ✕
                    </span>
                  </div>
        ...
        >
          <div
            ...
            style={{
              ...
              width: '100%', maxWidth: 380, border: '1px solid #e0dcd6'
            }}
          >
            <h2 style={{ fontSize: '1.1rem', fontWeight: 900, marginBottom: 16, color: '#1a1a2e' }}>
              📝 과목 추가
            </h2>
                ...
                {COLORS.map(c => (
                  <div
                    ...
                    style={{
                      ...
                      border: selectedColor === c ? '3px solid #1a1a2e' : '2px solid transparent',
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                ...
                style={{
                  ...
                  border: 'none', background: '#f0ede8', color: '#6b7280',
